#!/bin/bash
#SBATCH --job-name=Exp4_SegFormerB0
#SBATCH --partition=gpu              # adjust to your cluster partition
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=24:00:00
#SBATCH --output=%x_%j.log          # Exp4_SegFormerB0_<jobid>.log
# ═══════════════════════════════════════════════════════════════
#  Exp4_SegFormerB0: Full Pipeline Training + Evaluation
#  Model: SegFormer-B0 (MiT-B0 backbone + MLP decode head + VideoMAE + ATF)
#  Stage4 channels: 256 (MiT-B0 last stage)
#  Note: smallest model (~3.7M params), fastest training
# ═══════════════════════════════════════════════════════════════
set -e
...
EXP_DIR=$PROJECT/Exp4_SegFormerB0
cd $PROJECT

# ── Model selection ──
export MODEL_NAME=segformer_b0
export STAGE4_CHANNELS=256          # MiT-B0 last-stage channels
...
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 4: SegFormer-B0 (MiT-B0)"
echo "  MODEL_NAME=$MODEL_NAME  STAGE4_CHANNELS=$STAGE4_CHANNELS"
...
# ── STAGE 5: E2E ──
echo ""
echo ">>>>> STAGE 5: E2E (${E2E_EPOCHS} epochs) <<<<<"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 4: SegFormer-B0 — COMPLETE"
echo "  Finished: $(date)"

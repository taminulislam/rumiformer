#!/bin/bash
#SBATCH --job-name=Exp3_SAM2
#SBATCH --partition=gpu              # adjust to your cluster partition
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=48:00:00
#SBATCH --output=%x_%j.log          # Exp3_SAM2_<jobid>.log
# ═══════════════════════════════════════════════════════════════
#  Exp3_SAM2: Full Pipeline Training + Evaluation
#  Model: SAM2 (MobileViTv2-200 backbone + MLP decode head + VideoMAE + ATF)
#  Stage4 channels: 512 (MobileViTv2-200 last stage)
# ═══════════════════════════════════════════════════════════════
set -e
...
EXP_DIR=$PROJECT/Exp3_SAM2
cd $PROJECT

# ── Model selection ──
export MODEL_NAME=sam2
export STAGE4_CHANNELS=512          # MobileViTv2-200 last-stage channels
...
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 3: SAM2 (MobileViTv2-200)"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 3: SAM2 — COMPLETE"
echo "  Finished: $(date)"

#!/bin/bash
#SBATCH --job-name=Exp5_Prior2Former
#SBATCH --partition=gpu              # adjust to your cluster partition
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=48:00:00
#SBATCH --output=%x_%j.log          # Exp5_Prior2Former_<jobid>.log
# ═══════════════════════════════════════════════════════════════
#  Exp5_Prior2Former: Full Pipeline Training + Evaluation
#  Model: Prior2Former (PoolFormerV2-S24 backbone + MLP decode head + VideoMAE + ATF)
#  Stage4 channels: 512 (PoolFormerV2-S24 last stage)
# ═══════════════════════════════════════════════════════════════
set -e
...
EXP_DIR=$PROJECT/Exp5_Prior2Former
cd $PROJECT

# ── Model selection ──
export MODEL_NAME=prior2former
export STAGE4_CHANNELS=512          # PoolFormerV2-S24 last-stage channels
...
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 5: Prior2Former (PoolFormerV2-S24)"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 5: Prior2Former — COMPLETE"
echo "  Finished: $(date)"

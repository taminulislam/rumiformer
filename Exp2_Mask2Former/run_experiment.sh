#!/bin/bash
#SBATCH --job-name=Exp2_Mask2Former
#SBATCH --partition=gpu              # adjust to your cluster partition
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=48:00:00
#SBATCH --output=%x_%j.log          # Exp2_Mask2Former_<jobid>.log
# ═══════════════════════════════════════════════════════════════
#  Exp2_Mask2Former: Full Pipeline Training + Evaluation
#  Model: Mask2Former (SwinV2-Tiny backbone + MLP decode head + VideoMAE + ATF)
#  Stage4 channels: 768 (SwinV2-Tiny last stage)
# ═══════════════════════════════════════════════════════════════
set -e
...
EXP_DIR=$PROJECT/Exp2_Mask2Former
cd $PROJECT

# ── Model selection ──
export MODEL_NAME=mask2former
export STAGE4_CHANNELS=768          # SwinV2-Tiny last-stage channels
...
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 2: Mask2Former (SwinV2-Tiny)"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  Experiment 2: Mask2Former — COMPLETE"
echo "  Finished: $(date)"
